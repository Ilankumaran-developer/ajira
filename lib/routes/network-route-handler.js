...
class NetworkRouteHandler extends Graph{
    ...
    initGraph(){
        let devices = [{
            ...
            "body":{"type" : "COMPUTER", "name" : "C1"}
        },{
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C2"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C3"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C4"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C5"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C6"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C7"}
        },
        {
            "command":"CREATE /devices",
            "body":{"type" : "REPEATER", "name" : "R1"}
        },{
            "command":"CREATE /devices",
            "body":{"type" : "COMPUTER", "name" : "C10"}
        }]
        for(let device of devices){
            this.deviceProcessor(device)
        }
        let connections  = [{
            "command":"CREATE /connections",
            "body":{"source" : "C1", "targets" : ["C2", "C3"]}
        },{
            "command":"CREATE /connections",
            "body":{"source" : "C3", "targets" : ["R1"]}
        },{
            "command":"CREATE /connections",
            "body":{"source" : "R1", "targets" : ["C4"]}
        },{
            "command":"CREATE /connections",
            "body":{"source" : "C4", "targets" : ["C5", "C6"]}
        },
        {
            "command":"CREATE /connections",
            "body":{"source" : "C6", "targets" : ["C7"]}
        } ]
        for(let c of connections){
            this.connectionProcessor(c)
        }
    ...
    fetchRoutes(payload){
        ...
        console.log(_.isEmpty(me.networkGraph[to]), me.networkGraph[to])
        console.log(_.isEmpty(me.networkGraph[from]), me.networkGraph[from])
    ...
    connectDevices(connection){
        let me = this;
            ...
            console.log(source, 'source', connection.source, me.networkGraph)
            if(!_.isEmpty(source)){
                for(let target of connection.targets){
                    let targetInNet = me.networkGraph[target]
                    console.log(targetInNet, 'target', target)
